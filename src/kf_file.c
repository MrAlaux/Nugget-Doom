...
static void read_mobj_t(mobj_t *str, thinker_class_t tc, json_t *obj)
{
    ...
    // Alt. sprites
    str->altsprite = JS_GetIntegerValue(obj, "altsprite");
    str->altframe = JS_GetIntegerValue(obj, "altframe");

    // Alt. states
    JS_GetIdx(str->altstate, altstates, obj, "altstate");
    str->alttics = JS_GetIntegerValue(obj, "alttics");

    str->isvisual = JS_GetIntegerValue(obj, "isvisual");

    if (str->isvisual) { str->info = P_VisualMobjDummyInfo(); }

    str->gentranmap_pct = JS_GetIntegerValue(obj, "gentranmap_pct");
...
static json_mut_t *write_mobj_t(mobj_t *str, json_mut_doc_t *doc)
{
    ...
    // Alt. sprites
    JS_SetInt(doc, obj, "altsprite", str->altsprite);
    JS_SetInt(doc, obj, "altframe", str->altframe);

    // Alt. states
    JS_SetIdx(doc, obj, "altstate", str->altstate, altstates);
    JS_SetInt(doc, obj, "alttics", str->alttics);

    JS_SetInt(doc, obj, "isvisual", str->isvisual);
    JS_SetInt(doc, obj, "gentranmap_pct", str->gentranmap_pct);
...
static void read_pspdef_t(pspdef_t *str, json_t *obj)
{
    ...
    str->wix = JS_GetIntegerValue(obj, "wix");
    str->wiy = JS_GetIntegerValue(obj, "wiy");
    str->oldwix = JS_GetIntegerValue(obj, "oldwix");
    str->oldwiy = JS_GetIntegerValue(obj, "oldwiy");
}
...
static json_mut_t *write_pspdef_t(pspdef_t *str, json_mut_doc_t *doc)
{
    ...
    JS_SetInt(doc, obj, "wix", str->wix);
    JS_SetInt(doc, obj, "wiy", str->wiy);
    JS_SetInt(doc, obj, "oldwix", str->oldwix);
    JS_SetInt(doc, obj, "oldwiy", str->oldwiy);
...
static void read_player_t(player_t *str, json_t *obj)
{
    ...
    // [Nugget]
    str->jumptics = JS_GetIntegerValue(obj, "jumptics");
    str->crouchoffset = JS_GetIntegerValue(obj, "crouchoffset");
}
...
static json_mut_t *write_player_t(player_t *str, json_mut_doc_t *doc)
{
    ...
    // [Nugget]
    JS_SetInt(doc, obj, "jumptics", str->jumptics);
    JS_SetInt(doc, obj, "crouchoffset", str->crouchoffset);
...
static void ArchiveAutoMap(json_mut_doc_t *doc, json_mut_t *root_mut)
{
    ...
    if (markpointnum)
    {
        for (int i = 0; i < markpointnum; ++i)
        {
            ...
            JS_ArrayAddInt(doc, markpoint_arr, markpoints[i].pt.x);
            JS_ArrayAddInt(doc, markpoint_arr, markpoints[i].pt.y);

            // [Nugget]
            JS_ArrayAddInt(doc, markpoint_arr, markpoints[i].color);
...
static void UnArchiveAutoMap(json_t *root)
{
    ...
    if (markpointnum)
    {
        ...
        for (int i = 0; i < markpointnum; ++i)
        {
            ...
            // [Nugget] Changed struct
            markpoints[i].pt.x = JS_GetInteger(JS_GetArrayItem(markpoint_arr, 0));
            markpoints[i].pt.y = JS_GetInteger(JS_GetArrayItem(markpoint_arr, 1));

            // [Nugget]
            markpoints[i].color = JS_GetInteger(JS_GetArrayItem(markpoint_arr, 2));
        }
