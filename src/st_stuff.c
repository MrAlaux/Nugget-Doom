...
#define HU_FONTSIZE_EX (HU_FONTSIZE + HU_FONTEXTRAS)
                             ...
                             stringline_t *line)
{
    ...
    while (*str)
    {
        ...
        {
            int idx = (unsigned char) ST_ToUpper(ch) - HU_FONTSTART; // [Nugget] Cast to unsigned
            patch_t *patch =
                (idx < 0 || idx >= HU_FONTSIZE) ? NULL : font->characters[idx];

            // [Nugget] Support icons
            if (HU_FONTSIZE <= idx && idx < HU_FONTSIZE_EX)
            { patch = font_extras[idx - HU_FONTSIZE]; }
...
#define DrawPatch(x1, y1, x2, y2, dry, c, mh, a, p, cr, tl) \
  DrawPatchEx(x1, y1, x2, y2, dry, c, mh, a, p, cr, CR_NONE, tl)

static void DrawPatchEx(int x1, int y1, int *x2, int *y2, boolean dry,
                        crop_t crop, int maxheight, sbaralignment_t alignment,
                        patch_t *patch, crange_idx_e cr, crange_idx_e cr2, const byte *tl)
{
    if (!patch)
    {
        return;
    ...
    V_DrawPatchShadowed(x1, y1, xoffset, yoffset, crop, patch, false, outr, outr2, tl);
                          ...
                          patch_t *glyph)
{
    ...
    if (glyph)
    {
        ...
        DrawPatchEx(x1 + line->xoffset, y1, x2, y2, dry, no_crop,
